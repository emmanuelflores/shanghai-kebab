...
- (void)loadView {
    ...
    UIButton *appleButton = [UIButton buttonWithType:UIButtonTypeCustom];//UIButtonTypeCustom UIButtonTypeRoundedRect
    appleButton.frame = CGRectMake(79,194,170,44);
    [appleButton addTarget:self action:@selector(NativeWebViewPressed:) forControlEvents:UIControlEventTouchUpInside];
    [appleButton setTitle:@"the Apple way" forState:UIControlStateNormal];
    [appleButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [appleButton setContentHorizontalAlignment:UIControlContentHorizontalAlignmentLeft];
...
- (void)NativeWebViewPressed:(id)sender {
    NativeWebViewController *nvc = [[NativeWebViewController alloc]init];
